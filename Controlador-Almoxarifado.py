...
#Funções:
    ...
    def salvar_registro(self):
        ...
        try:
            ...
            insert_query = """
            INSERT INTO regristos (nome_equipamento, retidara, devolucao, statu, nome_pessoa)
            VALUES (?, GETDATE(), NULL, 'Pendente', ?)
            """
            for equipamento in equipamentos:
                cursor.execute(insert_query, equipamento, nome)
    ...
    def excluir_registro(self, id):
        ...
        if confirm:
            try:
                ...
                delete_query = "DELETE FROM regristos WHERE id = ?"
                cursor.execute(delete_query, id)
                conn.commit()
                messagebox.showinfo("Sucesso", "Registro excluído com sucesso!")
                self.carregar_registros()
    ...
    def alternar_status(self, id):
        try:
            ...
            cursor.execute("SELECT statu FROM regristos WHERE id = ?", id)
            status_atual = cursor.fetchone()[0]
...
if __name__ == "__main__":
    ...
    app = RegistroApp(root)
    root.mainloop()
>>>>>>> b56038da6f8ac776b1534736ce23fa841d369f36
